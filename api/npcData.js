...
const getNpc = (uid) => new Promise((resolve, reject) => {
  axios
    .get(`${dbUrl}/npc.json?orderBy="uid"&equalTo="${uid}"`)
    .then((response) => {
      if (response.data) {
...
const createNpc = (npcObj) => new Promise((resolve, reject) => {
  axios
    .post(`${dbUrl}/npc.json`, npcObj)
    .then((response) => {
      const payload = { firebaseKey: response.data.name };
      axios.patch(`${dbUrl}/npc/${response.data.name}.json`, payload).then(() => {
...
const deleteSingleNpc = (firebaseKey, uid) => new Promise((resolve, reject) => {
  axios
    .delete(`${dbUrl}/npc/${firebaseKey}.json`)
    .then(() => {
